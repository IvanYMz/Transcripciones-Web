...
const FileDropZone = ({ toggleShowFileDropzone, showFileDropzone, setSelectedFile, selectedFile, closeFilePreview }: FileDropzoneProps) => {
  ...
  return (
    <div className={'flex items-center justify-center text-[#fefefe] w-full h-full'}>
      ...
      {(selectedFile && !showFileDropzone) && (
        <Player closeFilePreview={closeFilePreview} selectedFile={selectedFile} />
      )}
    </div>
  );
};
